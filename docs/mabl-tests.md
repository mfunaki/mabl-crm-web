...
| production | https://mabl-crm-web-ixi7x7b23a-an.a.run.app |
...
| 順序 | テストラベル | 備考 |
|---|---|---|
| 1 | `ログイン - 正常系` | 認証機能の基本確認 |
| 2 | `ログイン - 異常系` | エラーハンドリングの確認 |
| 3 | `ダッシュボード - 統計表示` | ログイン後の基本画面確認 |
| 4 | `顧客一覧 - 検索・フィルター` | 検索・フィルター機能の確認 |
| 5 | `顧客 - 新規登録` | データ作成（6・7 の前提） |
| 6 | `顧客 - 情報編集` | 5 で作成したデータを編集 |
| 7 | `顧客 - 削除` | 6 で編集したデータを削除（後始末） |
| 8 | `ログアウト - 認証保護` | セッション管理の確認 |

> **注意:** シナリオ 5〜7 は本番 DB にテストデータ（「テスト 太郎」）を作成・削除します。
> シナリオ 7 で必ず削除されるため通常はデータが残りません。
> ただし、テスト途中で失敗した場合はデータが残る可能性があります。

**動作確認:** 2026-02-20 本番環境にて全テスト正常終了を確認済み。
